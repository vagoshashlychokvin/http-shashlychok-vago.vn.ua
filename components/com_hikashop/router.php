	...
	if($config->get('sef_remove_id',0) && isset($vars['ctrl']) && isset($vars['task'])){
		...
		if($config->get('alias_auto_fill',1)){
			...
			if($class->isMulti()){
				...
				$db->setQuery('SELECT reference_id FROM '.hikashop_table($trans_table,false).' WHERE reference_table='.$db->Quote($type).' AND reference_field='.$db->Quote($type.'_alias').' AND value = '.$db->Quote(str_replace(':','-',$name)));
		...
		if($class->isMulti()){
			...
			$db->setQuery('SELECT reference_id FROM '.hikashop_table($trans_table,false).' WHERE reference_table='.$db->Quote($type).' AND ((reference_field='.$db->Quote($type.'_alias').' AND value REGEXP '.$db->Quote($name_regex).') OR (reference_field='.$db->Quote($type.'_name').' AND value REGEXP '.$db->Quote($name_regex).'))');
			$retrieved_id = $db->loadResult();
			if($retrieved_id){
				$vars['cid'] = $retrieved_id;
				$vars['name'] = $name;
