...
class hikashopCartClass extends hikashopClass{
	...
	function get($cart_id=0,$keepEmptyCart=false, $cart_type='cart'){
		...
		if($this->hasCart($cart_id)){
			$filters=array('b.cart_id = '.(int)$this->cart->cart_id,'b.product_id > 0');
			hikashop_addACLFilters($filters,'product_access','c');
			$query='SELECT a.*,b.*,c.* FROM '.hikashop_table('cart').' AS a LEFT JOIN '.hikashop_table('cart_product').' AS b ON a.cart_id = b.cart_id LEFT JOIN '.hikashop_table('product').' AS c ON b.product_id=c.product_id WHERE '.implode(' AND ',$filters).' ORDER BY c.product_parent_id ASC,b.cart_product_modified ASC';
			$this->database->setQuery($query);
	...
	function save(&$cart){
		...
		$user = JFactory::getUser();
		$session = JFactory::getSession();
		if(!isset($cart->user_id) || !$app->isAdmin() || $currUser->user_cms_id == $cart->user_id || $session->getId()==$cart->session_id){
	...
	function delete(&$id, $cart_date='new'){
		...
		$user = JFactory::getUser();
		$session = JFactory::getSession();
		if($app->isAdmin() || $currUser->user_cms_id == $cartInfo->user_id || $session->getId()==$cartInfo->session_id){
