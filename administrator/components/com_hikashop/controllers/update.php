...
<?php
	...
	function wizard_save(){
		...
		if ($dataExample==1) //Install data sample
		{
			...
			$app->redirect('index.php?option=com_hikashop&ctrl=update&task=process_data_save&step=1');
		}
	...
	function process_data_save()
	{
		if (isset($_GET['step']))
		{
			...
			$url = 'index.php?option=com_hikashop&ctrl=update&task=process_data_save&step='.$step;
			...
			switch ($step)
			{
				...
				case 3 ://Copy
					...
					$dst = HIKASHOP_ROOT.'media\com_hikashop\upload\safe\\';

					$files = scandir($src,1);
					foreach($files as $f)
						if( $f!='..' and $f!='.' and !file_exists($dst.$f) )
						{
					...
					$dst = HIKASHOP_ROOT.'media\com_hikashop\upload\\';
			...
			$step++;
			if (!$error)
				$redirect = 'index.php?option=com_hikashop&ctrl=update&task=process_data_save&step='.$step;
			if ($error > 5)
