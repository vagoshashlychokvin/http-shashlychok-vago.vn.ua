...
class hikashopInstall{
	...
	function updateSQL(){
		...
		if(version_compare($this->fromVersion,'1.1.2','<')){
			$this->addColumns("product","`product_max_per_order` INT UNSIGNED DEFAULT 0");
		}

		if(version_compare($this->fromVersion,'1.3.4','<')){
			$this->addColumns("discount","`discount_auto_load` TINYINT UNSIGNED DEFAULT 0");
		}
		...
		if(version_compare($this->fromVersion,'1.4.2','<')){
			$this->addColumns("discount","`discount_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
			$this->addColumns("category","`category_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
			$this->addColumns("product","`product_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
			$this->addColumns("price","`price_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
			$this->addColumns("zone","`zone_currency_id` INT UNSIGNED DEFAULT 0");
		...
		if(version_compare($this->fromVersion,'1.4.5','<')){
			$this->addColumns("product",array("`product_group_after_purchase` VARCHAR( 255 ) NOT NULL DEFAULT ''","`product_contact` SMALLINT UNSIGNED DEFAULT 0"));
		}
		if(version_compare($this->fromVersion,'1.4.6','<')){
			...
			try{$this->db->query();}catch(Exception $e){}
			$this->addColumns("product","`product_min_per_order` INT UNSIGNED DEFAULT 0");
		}

		if(version_compare($this->fromVersion,'1.4.7','<')){
			$this->addColumns("payment","`payment_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
			$this->addColumns("shipping","`shipping_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
		}

		if(version_compare($this->fromVersion,'1.4.8','<')){
			$this->addColumns("history","`history_user_id` INT UNSIGNED DEFAULT 0");
			$this->addColumns("discount","`discount_tax_id` INT UNSIGNED DEFAULT 0");
			$this->addColumns("order",array("`order_discount_tax` decimal(12,5) NOT NULL DEFAULT '0.00000'","`order_shipping_tax` decimal(12,5) NOT NULL DEFAULT '0.00000'"));
		}

		if(version_compare($this->fromVersion,'1.4.9','<')){
			$this->addColumns("order","`order_number` VARCHAR( 255 ) NOT NULL DEFAULT ''");
		...
		if(version_compare($this->fromVersion,'1.5.0','<')){
			$this->addColumns("field","`field_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
			$this->addColumns("product","`product_min_per_order` INT UNSIGNED DEFAULT 0");
			if(version_compare(JVERSION,'1.6.0','<')){
			...
			$this->db->setQuery($query);
			try{$this->db->query();}catch(Exception $e){}
			$this->addColumns("discount",array("`discount_quota_per_user` INT UNSIGNED DEFAULT 0","`discount_minimum_products` INT UNSIGNED DEFAULT 0"));
		}

		if(version_compare($this->fromVersion,'1.5.2','<')){
			$this->addColumns("category","`category_keywords` VARCHAR(255) NOT NULL");
			$this->addColumns("category","`category_meta_description` varchar(155) NOT NULL DEFAULT ''");
			$this->addColumns("product_related","`product_related_ordering` INT UNSIGNED DEFAULT 0");
			$this->addColumns("product","`product_last_seen_date` INT UNSIGNED DEFAULT 0");
			$this->addColumns("file","`file_free_download` tinyint(3) unsigned NOT NULL DEFAULT '0'");
...
CREATE TABLE IF NOT EXISTS `#__hikashop_limit` (
	...
	PRIMARY KEY (`limit_id`)
			...
			try{$this->db->query();}catch(Exception $e){}
			$this->addColumns("zone","INDEX ( `zone_code_3` )");
			$this->addColumns("product","`product_sales` INT UNSIGNED DEFAULT 0");
			$this->addColumns("field",array("`field_with_sub_categories` TINYINT( 1 ) NOT NULL DEFAULT 0","`field_categories` VARCHAR( 255 ) NOT NULL DEFAULT 'all'"));
			$this->addColumns("payment","`payment_shipping_methods` TEXT NOT NULL DEFAULT  ''");
			$this->addColumns("cart_product","`cart_product_option_parent_id` INT UNSIGNED DEFAULT 0");
			$this->addColumns("order_product","`order_product_option_parent_id` INT UNSIGNED DEFAULT 0");
			$this->addColumns("taxation","`taxation_access` VARCHAR( 255 ) NOT NULL DEFAULT 'all'");
...
CREATE TABLE IF NOT EXISTS `#__hikashop_filter` (
	...
	PRIMARY KEY (`filter_id`)
			...
			$this->addColumns("payment","`payment_currency` VARCHAR( 255 ) NOT NULL");
		}
...
CREATE TABLE IF NOT EXISTS `#__hikashop_waitlist` (
	...
	PRIMARY KEY (`waitlist_id`)
			...
			$this->addColumns("product","`product_waitlist` SMALLINT( 5 ) UNSIGNED NOT NULL DEFAULT '0'");
			$this->addColumns("discount","`discount_coupon_nodoubling` TINYINT NULL;");
			$this->addColumns("discount","`discount_coupon_product_only` TINYINT NULL;");
		}

		if(version_compare($this->fromVersion,'1.5.6','<')){
			$this->addColumns("taxation","`taxation_cumulative` TINYINT NULL;");
			$this->addColumns("order","`order_tax_info` text NOT NULL");
			$this->addColumns("order_product","`order_product_tax_info` text NOT NULL");
			$this->addColumns("category","`category_layout` varchar(255) NOT NULL DEFAULT ''");
			$this->addColumns("product","`product_layout` varchar(255) NOT NULL DEFAULT ''");
		}
		if(version_compare($this->fromVersion,'1.5.7','<')){
			$this->addColumns("characteristic","`characteristic_alias` varchar(255) NOT NULL DEFAULT ''");
			$this->addColumns("product",array("`product_average_score` FLOAT NOT NULL","`product_total_vote` INT NOT NULL DEFAULT '0'"));
			$this->addColumns("address","`address_default` TINYINT NOT NULL DEFAULT '0'");
			$this->addColumns("file",array("`file_ordering` INT UNSIGNED NOT NULL DEFAULT 0","`file_limit` INT NOT NULL DEFAULT 0"));
			$this->db->setQuery("
...
CREATE TABLE IF NOT EXISTS `#__hikashop_vote` (
	...
	PRIMARY KEY (`vote_id`)
		...
		if(version_compare($this->fromVersion,'1.5.8','<')){
			...
			try{$this->db->query();}catch(Exception $e){}
			$this->addColumns("order","`order_payment_price` decimal(17,5) NOT NULL DEFAULT '0.00000'");
			$this->addColumns("payment","`payment_price` decimal(17,5) NOT NULL DEFAULT '0.00000'");
		}
...
CREATE TABLE IF NOT EXISTS `#__hikashop_shipping_price` (
	...
	PRIMARY KEY (`shipping_price_id`)
			...
			try{$this->db->query();}catch(Exception $e){}
			$this->addColumns("product","`product_page_title` varchar(255) NOT NULL DEFAULT ''");
			$this->addColumns("category","`category_page_title` varchar(255) NOT NULL DEFAULT ''");
			$this->addColumns("characteristic","`characteristic_ordering` INT( 12 ) UNSIGNED NOT NULL DEFAULT '0' AFTER  `characteristic_alias`");
...
CREATE TABLE IF NOT EXISTS `#__hikashop_badge` (
	...
	PRIMARY KEY (`badge_id`)
			...
			$this->addColumns("cart",array("`cart_type` varchar(25) NOT NULL DEFAULT 'cart'",
					"`cart_name` varchar(50) NOT NULL",
					"`cart_share` varchar(255) NOT NULL DEFAULT 'nobody'",
					"`cart_current` INT NOT NULL DEFAULT '0'"));

			$this->addColumns("cart_product","`cart_product_wishlist_id` INT NOT NULL DEFAULT '0'");
			$this->addColumns("order_product","`order_product_wishlist_id` INT NOT NULL DEFAULT '0'");

			$this->addColumns("widget",array("`widget_published` tinyint(4) NOT NULL DEFAULT 1",
		...
		if(version_compare($this->fromVersion,'1.6.0','<')){
			$this->addColumns("address","`address_street2` TEXT NOT NULL");
		}

		if(version_compare($this->fromVersion,'2.0.0','<')){
			$this->addColumns("order",array("`order_invoice_number` VARCHAR( 255 ) NOT NULL DEFAULT ''","`order_invoice_id` INT NOT NULL DEFAULT '0'"));
			...
			try{$this->db->query();}catch(Exception $e){}
			$this->addColumns("download","`file_pos` int(10) NOT NULL DEFAULT '1'");
			$this->db->setQuery("ALTER TABLE `#__hikashop_download` DROP PRIMARY KEY , ADD PRIMARY KEY ( `file_id` , `order_id` , `file_pos` );");
			try{$this->db->query();}catch(Exception $e){}
			$this->addColumns("product_category`","`product_parent_id` INT NOT NULL DEFAULT '0'");
		...
		if(version_compare($this->fromVersion,'2.1.0','<')){
			$this->addColumns("product","`product_alias` VARCHAR( 255 ) NOT NULL DEFAULT ''");
			$this->addColumns("category","`category_alias` VARCHAR( 255 ) NOT NULL DEFAULT ''");

			if($this->level=='starter'){
				$this->db->setQuery("DELETE FROM `#__hikashop_widget` ;");
				try{$this->db->query();}catch(Exception $e){}
			}
			$this->addColumns("order","`order_invoice_created` INT(10) UNSIGNED NOT NULL DEFAULT '0'");
		...
		if(version_compare($this->fromVersion,'2.1.1','<')){
			$this->addColumns("product","`product_price_percentage` decimal(15,7) NOT NULL DEFAULT '0'");
			$this->addColumns("discount","`discount_affiliate` INT(10) NOT NULL DEFAULT '0'");
			$this->addColumns("badge","`badge_keep_size` INT(10) NOT NULL DEFAULT '0'");
		}
		if(version_compare($this->fromVersion,'2.1.2','<')){
			$this->addColumns("product",array("`product_canonical` VARCHAR( 255 ) NOT NULL DEFAULT ''","`product_msrp` decimal(15,7) NULL DEFAULT '0'"));
			$this->addColumns("badge","`badge_quantity` VARCHAR( 255 ) NULL DEFAULT ''");
			$this->addColumns("category",array("`category_canonical` VARCHAR( 255 ) NOT NULL DEFAULT ''","`category_site_id` VARCHAR( 255 ) NULL DEFAULT ''"));
		}
		if(version_compare($this->fromVersion, '2.2.0', '<')) {
			$this->addColumns("payment",array("`payment_ordering` int(10) unsigned NOT NULL DEFAULT '0'",
				"`payment_published` tinyint(4) NOT NULL DEFAULT '1'"));

			$this->db->setQuery("ALTER TABLE `#__hikashop_payment` DROP INDEX payment_type");
			try{$this->db->query();}catch(Exception $e){}

			$this->addColumns("order",array("`order_shipping_params` text NOT NULL DEFAULT ''",
				"`order_payment_params` text NOT NULL DEFAULT ''"));

			$this->addColumns("order_product",array(
...
CREATE TABLE IF NOT EXISTS `#__hikashop_plugin` (
	...
	PRIMARY KEY (`plugin_id`)
			...
			$this->addColumns("field","`field_display` text NOT NULL DEFAULT ''");
			$this->addColumns("badge","`badge_url` VARCHAR( 255 ) NULL DEFAULT ''");
		}
		if(version_compare($this->fromVersion, '2.2.2', '<')) {
			$this->addColumns("taxation","`taxation_post_code` VARCHAR( 255 ) NULL DEFAULT ''");
			$this->addColumns("product","`product_display_quantity_field` SMALLINT DEFAULT 0");
		...
		if(version_compare($this->fromVersion, '2.2.3', '<')) {
			$this->addColumns("cart","`cart_params` text NOT NULL DEFAULT ''");
		}
		if(version_compare($this->fromVersion, '2.3.0', '<')) {
			$this->addColumns("taxation",array("`taxation_date_start` int(10) unsigned NOT NULL DEFAULT '0'","`taxation_date_end` int(10) unsigned NOT NULL DEFAULT '0'"));
			$this->db->setQuery("
			...
			$this->addColumns("product","`product_warehouse_id` int(10) unsigned NOT NULL DEFAULT '0'");
		...
		if(version_compare($this->fromVersion, '2.3.1', '<')) {
			$this->addColumns("product","`product_quantity_layout` varchar(255) NOT NULL DEFAULT ''");
			$this->addColumns("category","`category_quantity_layout` varchar(255) NOT NULL DEFAULT ''");
		}
		if(version_compare($this->fromVersion, '2.3.2', '<')) {
			$this->addColumns("order","`order_site_id` varchar(255) NOT NULL DEFAULT ''");
			$this->addColumns("price","`price_site_id` varchar(255) NOT NULL DEFAULT ''");

			$this->addColumns("characteristic",array(
			...
			foreach($elements as $element){
				...
				if(!empty($menu_id)){
					if($element->alias == 'hikashop-menu-for-brands-listing'){
						...
						$menusClass->attachAssocModule($menu_id, $display);
