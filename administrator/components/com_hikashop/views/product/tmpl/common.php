...
?>				<tr class="hikashop_product_quantity_row">
				...
				<tr class="hikashop_product_type_row">
					...
					<td>
						<?php if(!empty($this->element->product_weight) && hikashop_toFloat($this->element->product_weight) != 0.0){$product_type='shippable';}else{$product_type='virtual';}
						$arr = array(
							JHTML::_('select.option', 'shippable', JText::_('WIZARD_REAL')),
							JHTML::_('select.option', 'virtual', JText::_('WIZARD_VIRTUAL'))
						);
						echo JHTML::_('hikaselect.genericlist', $arr, "product_type" , ' onchange="hikashopSetShippable(this.value);"','value', 'text', @$product_type); ?>
						...
						window.hikashop.ready(function(){ hikashopSetShippable('<?php echo $product_type; ?>'); });
						</script>
					</td>
				</tr>
