...
function CreateZoneMore(zone, element, id, zoneBoutonSuppression){
	...
	jQuery(btnPlus).addClass("acyeditor_btnplus");
	btnPlus.onclick = function (evt) {
...
function addActionsButtons(id, zoneBoutonSuppression, evt){
	...
	if (evt != null && evt != undefined){
		if(evt.clientX + 100 <= zoneBody.clientWidth) zoneAction.style.left = evt.clientX - 100 + "px";
		else zoneAction.style.left = zoneBody.clientWidth - 200 + "px";
		zoneAction.style.top = evt.clientY - (evt.layerY != undefined && evt.layerY > 0 && evt.layerY != evt.clientY? evt.layerY: evt.offsetY) + "px";
	}
	zoneAction.id = "zoneAction";
	zoneAction.className = "acyeditor_action";

	var zoneCopyButton = document.createElement("div");
	zoneCopyButton.title = titleBtnDupliBefore;
	zoneCopyButton.id = "zoneCopyButton";
	zoneCopyButton.className = "acyeditor_copyButton acyeditor_copyButtonBefore";
	zoneCopyButton.onclick = function(){ duplicateZone(id, zoneBoutonSuppression, 'before'); }
	...
	zoneAction.appendChild(zoneCopyButton);
...
function hideActionButtons(id, e, zoneClick, protectEditor){
	...
	if(zoneClick == 'outside' || zoneClick == 'noClick' || canHide){
		...
		zoneBody.childNodes[0].className = "";
		var zoneGlob = zoneBody.getElementsByTagName('*');
		jQuery(zoneGlob).remove('.acyeditor_action');
